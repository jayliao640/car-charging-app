...
window.addEventListener("error", event => {
  ...
  if ($("syncStatus")) {
    $("syncStatus").textContent = "程式載入異常，請重新整理或重新上傳 index.html 與 app.js";
  }
});
window.addEventListener("unhandledrejection", event => {
  console.error(event.reason);
  if ($("syncStatus")) {
    $("syncStatus").textContent = "程式執行異常：" + (event.reason?.message || event.reason || "未知錯誤");
  }
});
...
function render(){
  ...
  if ($("countStat")) $("countStat").textContent=`${list.length} 次`;
  if ($("kwhStat")) $("kwhStat").textContent=`${total.toFixed(2)} kWh`;
  if ($("avgStat")) $("avgStat").textContent=`${avg.toFixed(2)} kWh`;
  if ($("rangeTotalKwh")) $("rangeTotalKwh").textContent=`${total.toFixed(2)} kWh`;
  if ($("rangeSummaryText")) $("rangeSummaryText").textContent=`${list.length} 次充電｜平均 ${avg.toFixed(2)} kWh／次`;
...
async function migrateLocalToCloudOnce(){
  ...
  const migrationKey = "carChargingCloudMigrationV1";
  if(localStorage.getItem(migrationKey) === "done") return;

  const localRecords = JSON.parse(localStorage.getItem(LOCAL_KEY) || "[]");
  if(!localRecords.length){
    localStorage.setItem(migrationKey, "done");
    return;
  }

  setStatus(`發現 ${localRecords.length} 筆本機紀錄，正在同步到雲端…`);
  for(const record of localRecords){
    const cloudRecord = {
      ...record,
      id: record.id || ((window.crypto && typeof window.crypto.randomUUID === "function") ? window.crypto.randomUUID() : String(Date.now()+Math.random())),
      photoUrl: record.photoUrl || "",
      migratedAt: new Date().toISOString()
    };
    await firebaseFns.setDoc(
      firebaseFns.doc(db, "sharedRecords", cloudRecord.id),
      cloudRecord,
      {merge:true}
    );
  }
...
async function loadCloud(){
  ...
  saveLocal();render();setStatus("雲端同步正常｜iPhone、Android 與電腦共用同一份紀錄");
}
async function initFirebase(){
  if(!configured()){
    setStatus("尚未連接 Firebase：目前資料只存在這台裝置，電腦與手機不會同步");
    render();return;
  }
  try{
...
function updateMainKwh(){
  if ($("calcKwh") && $("soc")) {
    $("calcKwh").textContent = calcKwh($("soc").value).toFixed(2) + " kWh";
  }
}
$("soc").addEventListener("input", updateMainKwh);
$("soc").addEventListener("change", updateMainKwh);
$("photo").onchange=e=>{
